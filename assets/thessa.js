document.addEventListener("DOMContentLoaded", function () {
    ...
    wordInput.addEventListener("keypress", (e) => { if (e.key === "Enter") handleGeneration(); });
    document.getElementById("thessa-info-btn").addEventListener("click", () => showModal(aboutModal));
    document.getElementById("settings-btn").addEventListener("click", () => {
        apiKeyInput.value = userApiKey || "";
        showModal(apiKeyModal);
    });
    document.getElementById("about-modal-close-btn").addEventListener("click", () => hideModal(aboutModal));
    document.getElementById("apiKeyModalCloseBtn").addEventListener("click", () => hideModal(apiKeyModal));
    document.getElementById("saveApiKeyBtn").addEventListener("click", () => saveApiKey(apiKeyInput.value));
    ...
    async function fetchWordData(word) {
        ...
        const loadingMessages = ["Contacting Gemini...", "Analyzing syllables...", "Consulting ancient texts...", "Reticulating splines...", "Polishing results..."];
        let messageIndex = 0;
        ...
        try {
            const response = await fetch(getApiUrl(), {
                ...
                body: JSON.stringify({
                    ...
                    generationConfig: { temperature: 0.6, maxOutputTokens: 4096, response_mime_type: "application/json" },
                }),
            });
    ...
    function showHoverPopover(element) {
        ...
        hoverPopover.style.left = `${rect.left + rect.width / 2 - hoverPopover.offsetWidth / 2}px`;
        hoverPopover.style.top = `${rect.top - hoverPopover.offsetHeight - 10}px`;
    }

    function hideHoverPopover() { if(hoverPopover) hoverPopover.classList.remove("visible"); }

    function copyToClipboard(text) {
        navigator.clipboard.writeText(text).then(() => {
            if(!copyNotificationEl) return;
            copyNotificationEl.textContent = `Copied: "${text.substring(0, 25)}..."`;
    ...
    function setGenerateBtnLoading(isLoading) {
        ...
        generateBtn.innerHTML = isLoading ? `<div class="loading-spinner" style="width:24px; height:24px; border-width:2px; margin:0;"></div>` : `<svg class="btn-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>`;
    }

    function loadApiKey() {
        const storedKey = localStorage.getItem(API_KEY_STORAGE_KEY);
        if (storedKey) { userApiKey = storedKey; return true; }
        return false;
    }
